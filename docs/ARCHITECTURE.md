...
| `ToolRegistry` / `ToolLifecycle` | Propose -> approve/reject -> execute -> recover/abandon for controlled external actions, gated by action level, a kill switch, budgets, scheduling, and content safety gates. Public publishing may be auto-approved only by the recorded AION autonomy policy after those gates pass. |
