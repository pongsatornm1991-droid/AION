...
def _platform_operations_snapshot(memory, reels):
    ...
    return {
        "platforms": [
            ...
            {"id": "youtube", "name": "YouTube", "color": "#ff5a63", "purpose": "Shorts เพื่อการค้นพบ และตอนหลักเพื่อเล่าเรื่องลึก", "cadence": "ผลิต จ.–พ. · Shorts พฤ./ศ./อา. 20:43 · ตอนหลัก ส. 20:15 (เวลาไทย)", "automation": "Research → Studio → Video QA → YouTube → Cross-post", "community": "วิเคราะห์ retention และคำถามผู้ชมเมื่อมีข้อมูล", "messages": "ไม่มี DM ใน workflow", "published": reels["platform_counts"].get("youtube", 0)},
        ],
        "social_team": social,
        "admin_team": admin,
...
def _next_studio_release(memory):
    ...
    """
    try:
        # Prefer the current Studio episode.  A historical Reel is never a
        # substitute for a newly produced Creator episode in this view.
        creator = YouTubeCreatorQueue(memory, ROOT)
        for item in creator.candidates():
            if item.get("content_kind") != "short" or item.get("status") not in {"upload-ready", "already-prepared"}:
                continue
            video_path = str(item.get("video_path") or "").replace("\\", "/").lstrip("/")
            ...
            return {
                ...
                "boundary": item.get("uncertainty_boundary") or "",
                "quality": "ผ่าน Quality Gate · ผลิตใหม่จาก AION Studio",
            }
