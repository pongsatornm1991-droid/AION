...
class PostPhotoToFacebookTests(unittest.TestCase):
    """Offline tests for post_photo_to_facebook() (built 2026-09-03 so
    VisualContentCycle can cross-post the same image+caption it sends
    to Instagram to the Facebook Page too)."""
    ...
    def test_empty_image_url_is_rejected_before_any_network_call(self):
        with mock.patch("requests.post") as mock_post:
            with self.assertRaises(ValueError):
                post_photo_to_facebook("   ")
            mock_post.assert_not_called()
    ...
    def test_missing_access_token_raises_a_clear_error(self):
        with mock.patch.dict(os.environ, {"FACEBOOK_PAGE_ACCESS_TOKEN": ""}):
            with mock.patch("requests.post") as mock_post:
                with self.assertRaises(RuntimeError) as ctx:
                    post_photo_to_facebook("https://example.com/a.png", access_token=None)
                mock_post.assert_not_called()
        self.assertIn("FACEBOOK_PAGE_ACCESS_TOKEN", str(ctx.exception))
    ...
    def test_missing_page_id_raises_a_clear_error(self):
        with mock.patch.dict(os.environ, {"FACEBOOK_PAGE_ID": ""}):
            with mock.patch("requests.post") as mock_post:
                with self.assertRaises(RuntimeError) as ctx:
                    post_photo_to_facebook("https://example.com/a.png", page_id=None)
                mock_post.assert_not_called()
        self.assertIn("FACEBOOK_PAGE_ID", str(ctx.exception))
    ...
    def test_successful_post_returns_the_graph_api_payload(self):
        with mock.patch(
            "requests.post",
            return_value=FakeResponse(200, {"id": "999_888", "post_id": "111_222"}),
        ) as mock_post:
        ...
        self.assertEqual(result, {"id": "999_888", "post_id": "111_222"})
        called_url = mock_post.call_args.args[0]
        self.assertEqual(called_url, f"{GRAPH_API_BASE}/test-page-id/photos")
        called_data = mock_post.call_args.kwargs["data"]
        self.assertEqual(
            called_data["url"],
            "https://raw.githubusercontent.com/x/y/main/content/images/a.png",
    ...
    def test_explicit_credentials_override_environment(self):
        with mock.patch(
            ...
            post_photo_to_facebook(
                ...
                access_token="explicit-token", page_id="explicit-page",
            )

        called_url = mock_post.call_args.args[0]
        called_data = mock_post.call_args.kwargs["data"]
        self.assertEqual(called_url, f"{GRAPH_API_BASE}/explicit-page/photos")
        self.assertEqual(called_data["access_token"], "explicit-token")
    ...
    def test_http_error_status_raises_runtime_error_with_details(self):
        ...
        with mock.patch(
            ...
            with self.assertRaises(RuntimeError) as ctx:
                post_photo_to_facebook("https://example.com/a.png")
