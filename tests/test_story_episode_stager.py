...
class StoryEpisodeStagerTests(unittest.TestCase):
    def test_stages_one_traceable_subject_first_short(self):
        with tempfile.TemporaryDirectory() as root:
            ...
            self.assertEqual(5, len(episode["scenes"]))
        ...
        with tempfile.TemporaryDirectory() as root:
            ...
            for index in range(2):
                (series / f"episode-{index}.json").write_text(json.dumps({
                    ...
                    "target_duration_seconds": 15, "scene_seconds": 5, "pacing_policy": "fast-cut-subject-first-v1",
                    ...
                    "scenes": [{"n": number, "visual": "AION appears briefly while the subject dominates the frame.", "narration": "AION asks a careful question."} for number in range(1, 4)],
                }), encoding="utf-8")
            made = []
